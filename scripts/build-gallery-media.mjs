...
`;
  ...
  /**
   * Provenance, kept on the server.
   *
   * `gallery.generated.ts` is imported by a client component, so anything in
   * it ships to the browser. Provider identities must not: "many vendors, one
   * interface" stops being true the moment a visitor can read which vendor
   * made a card, and `tests/unit/gallery.test.ts` fails the build over it.
   *
   * So the model each card came from lives here instead, server-only, where
   * the publication-policy check can read it and the bundle cannot.
   */
...
export const GALLERY_SOURCE_MODELS: Readonly<Record<string, string>> = ${JSON.stringify(
    Object.fromEntries(selection.map((item) => [item.id, item.sourceModel])),
    null,
    2,
  )};
`;
  ...
  writeFileSync(
    path.join(ROOT, "services", "marketing", "gallery-provenance.generated.ts"),
    provenance,
  );
